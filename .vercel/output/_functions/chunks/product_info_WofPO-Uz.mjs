import { a as getProductById, g as getProducts } from './catalog_Cg0LSCvE.mjs';
...
const productContentMap = {
  faja_chaleco_cinturilla: {
    ...
    intro: "Una prenda pensada para abrazar el torso, definir la silueta y mantener una sensacion de ajuste agradable durante el dia.",
    longDescription: "Esta faja chaleco cinturilla combina estructura y suavidad para que puedas realzar cintura, abdomen y espalda sin renunciar a la comodidad visual y corporal. Su diseno busca ofrecer una base firme para looks diarios o momentos especiales.",
    benefits: [
  ...
  cinturilla_reloj_arena: {
    ...
    intro: "Ideal para quienes buscan una prenda mas centrada en la zona media y un look estilizado.",
    longDescription: "Su estructura prioriza la definicion de cintura y una presencia visual mas marcada, manteniendo una estetica limpia y facil de integrar en el catalogo.",
    benefits: [
  ...
  faja_moldeadora: {
    ...
    longDescription: "La faja short moldeadora esta pensada para acompanar abdomen, cintura, cadera y muslos con una compresion equilibrada. Su formato tipo short ayuda a crear una linea continua bajo vestidos, faldas o pantalones.",
    benefits: [
...
function mergeGallery(product, gallery) {
...
function getProductPageById(id) {
...
function getRelatedProducts(productId) {
  ...
  return pageData.relatedProductIds.map((id) => getProducts().find((product) => product.id === id)).filter((product) => Boolean(product)).map((product) => ({
    ...product,
    image: normalizeImagePath(product.image)
  }));
}
Object.keys(productContentMap).map((id) => getProductPageById(id)).filter((product) => Boolean(product));

export { getRelatedProducts as a, getProductPageById as g };
