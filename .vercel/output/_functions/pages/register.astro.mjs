...
const $$ConfRegister = createComponent(async ($$result, $$props, $$slots) => {
  return renderTemplate`${maybeRenderHead()}<section class="login-shell"> <div class="login-glow login-glow-left" aria-hidden="true"></div> <div class="login-glow login-glow-right" aria-hidden="true"></div> <div class="login-layout"> <aside class="login-copy"> <p class="login-eyebrow">CurvySweet</p> <h1>Registrate de forma sencilla</h1> <p class="login-description">
...
Registrarse
</button> </div> <p class="auth-subtitle" data-auth-subtitle>Introduzca sus datos</p> </div> <div class="auth-panels"> <form class="login-form auth-panel is-active" data-auth-panel="login" method="POST" action="/api/login"> <div class="input-grid"> <label class="input-box"> <span>Nombre</span> <input type="text" placeholder="Tu nombre"> </label> <label class="input-box"> <span>Apellido</span> <input type="text" placeholder="Tu apellido"> </label> </div> <label class="input-box"> <span>Usuario</span> <input type="text" placeholder="@curvysweet"> </label> <label class="input-box"> <span>Email</span> <input name="mail" type="email" placeholder="correo electronico" required> </label> <label class="input-box"> <span>Contrasena</span> <input name="password" type="password" placeholder="********" required> </label> <div class="remember-forgot"> <label class="remember-option"> <input type="checkbox"> <span>Recordarme</span> </label> <a href="#">Has olvidado tu contrasena?</a> </div> <button type="submit" class="btn">Entrar</button> <p class="form-message" data-login-message aria-live="polite"></p> <div class="register-link"> <span>Aun no tienes cuenta?</span> <button type="button" class="text-link" data-auth-target="register">
Crear perfil
...
Entrar aqui
</button> </div> </form> </div> </div> </div> </section> <button id="theme-toggle" type="button">Modo oscuro</button> ${renderScript($$result, "C:/Users/Usuario/Escritorio/CurvySweet Project/zapping-zero/src/components/conf_register.astro?astro&type=script&index=0&lang.ts")}`;
}, "C:/Users/Usuario/Escritorio/CurvySweet Project/zapping-zero/src/components/conf_register.astro", void 0);

const $$Register = createComponent(($$result, $$props, $$slots) => {
  return renderTemplate`<html lang="es"> <head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>Registrarse</title>${renderHead()}</head> <body> ${renderComponent($$result, "Register_page", $$ConfRegister, {})} </body></html>`;
}, "C:/Users/Usuario/Escritorio/CurvySweet Project/zapping-zero/src/pages/register.astro", void 0);

const $$file = "C:/Users/Usuario/Escritorio/CurvySweet Project/zapping-zero/src/pages/register.astro";
const $$url = "/register";
